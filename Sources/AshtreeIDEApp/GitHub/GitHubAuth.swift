// GitHubAuth.swift v3 — Canonical Apple Sign In (crash fix for iOS 26 beta)
// Key fix: retain ASAuthorizationController strongly + main thread delegate dispatch
import Foundation
...
public final class IDEAuthViewModel: NSObject, ObservableObject {
    ...
    // CRITICAL: retain the controller so it lives until callback fires
    // This is the primary cause of the crash on iOS 26 beta
    private var _appleController: ASAuthorizationController?

    // Stored raw nonce for the current Apple Sign In request
    ...
    // MARK: Sign in with Apple (canonical iOS crash-free implementation)
    // Fix: nonce must be SHA256 of raw nonce; controller MUST be retained
    public func signInWithApple() {
        ...
        // Apple validates: SHA256(rawNonce) == nonce in the identity token JWT
        request.nonce = sha256(rawNonce)

        // Create and RETAIN the controller
        let controller = ASAuthorizationController(authorizationRequests: [request])
        controller.delegate = self
        controller.presentationContextProvider = self
        _appleController = controller  // retain!
        controller.performRequests()
    }

    public func startGitHubDeviceFlow() async {
        // Silent re-auth if token exists
        ...
        do {
            ...
            deviceFlow?.error=error.localizedDescription; deviceFlow?.isPolling=false
        }
    }
...
extension IDEAuthViewModel: ASAuthorizationControllerDelegate {
    public func authorizationController(controller:ASAuthorizationController,didCompleteWithAuthorization auth:ASAuthorization) {
        // Dispatch to main thread explicitly — crucial for iOS 26 beta
